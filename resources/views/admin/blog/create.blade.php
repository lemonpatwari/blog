                                ...
                                <form action="{{ route('blogs.store') }}" method="post" enctype="multipart/form-data">
                                    ...
                                    <div class="flex justify-between space-x-4 px-3 py-3">
                                        <div class="relative w-full px-2">
                                            ...
                                            <x-jet-input id="name" class="block mt-1 w-full py" placeholder="Enter name"
                                                         type="text"
                                                         name="name"
                                                         value="{{ old('name') }}"
                                                         required/>
                                        ...
                                        <div class="relative  w-full px-2">
                                            ...
                                            <select required id="is_approved" name="is_approved"
                                                ...
                                                <option value="1" {{ (1 == old('is_approved')) ? 'selected' :'' }}"
                                                }}>Yes</option>
                                                <option value="0" {{ (0 == old('is_approved')) ? 'selected' :'' }}"
                                                }}>No</option>
                                    ...
                                    <div class="flex justify-between space-x-4 px-3 py-3">
                                        <div class="relative  w-full px-2">
                                            ...
                                            <x-jet-input id="banner_url" class="block mt-1 w-full" type="file"
                                                         name="banner_url" accept="image/*" required/>
                                            @error('banner_url') <span
                                        ...
                                        <div class="relative  w-full px-2">
                                            ...
                                            <select id="tags_id" required multiple="multiple" name="tags_id[]"
                                                ...
                                                @foreach($tags as $key=>$row)
                                                    <option
                                                        value="{{ $key }}" {{ (collect(old('tags_id'))->contains($key)) ? 'selected':'' }}>{{ $row }}</option>
                                                @endforeach
                                            </select>

                                            @error('tags_id') <span class="text-red-500">{{ $message }}</span> @enderror
                                        </div>
                                        ...
                                        <div class="relative  w-full px-2">
                                            ...
                                            <select id="categories_id" multiple="multiple" name="categories_id[]"
                                                ...
                                                @foreach($categories as $key=>$row)
                                                    <option
                                                        value="{{ $key }}" {{ (collect(old('categories_id'))->contains($key)) ? 'selected':'' }}>{{ $row }}</option>
                                                @endforeach
                                            </select>

                                            @error('categories_id') <span
                                                class="text-red-500">{{ $message }}</span> @enderror
                                    ...
                                    <div class="flex justify-between space-x-4 px-3 py-3">
                                        ...
                                        <div class="relative  w-full px-2">
                                            ...
                                            <textarea name="blog_body" id="editor" cols="30" rows="10"
                                                      class="form-control editor">{{ old('blog_body') }}</textarea>
                                            @error('blog_body') <span
                                                class="text-red-500">{{ $message }}</span> @enderror
                                        </div>
